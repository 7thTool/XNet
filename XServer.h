...
typedef std::weak_ptr<wss_clt_t> wss_clt_weak_ptr; 
  ...
  public:
	XServiceT()
		: keepalive_(60), max_buffer_size_(4096), log_directory_("./log/"), ssl_directory_("./"), root_directory_("./www/"), io_thread_num_(0), io_service_(), io_ssl_context_(boost::asio::ssl::context::sslv23), stop_flag_(true), peer_id_(0)
	{
	}
	...
	bool start(int io_thread)
	{
		...
		peer_id_ = std::max(io_thread_num_, 1);
		...
		router_ = std::make_shared<Router>(*static_cast<T*>(this));
	...
	size_t connect(const x_char_t *addr, const x_ushort_t port, const x_int_t type, const x_size_t io_channel)
	{
		if (!is_run())
		{
			return 0;
	...
	void on_io_upgrade(ws_ptr peer_ptr)
	{
		...
		router_->on_io_upgrade(peer_ptr);
	}
	...
	on_io_read(http_ptr peer_ptr,
			   ...
			   Send &&send)
	{
		//router_->on_io_read(peer_ptr, doc_root, req, send);
	}
	template <class Body, class Allocator,
			  class Send>
	void
	on_io_read(https_ptr peer_ptr,
			   ...
			   Send &&send)
	{
		//router_->on_io_read(peer_ptr, std::forward(doc_root), std::forward(req), std::forward(send));
	}
	...
	{
		router_->on_io_activity(peer_ptr);
	}
